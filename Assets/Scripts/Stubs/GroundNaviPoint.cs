...
public class GroundNaviPoint : MonoBehaviour
{
    ...
    public void PrintPoints()
    {
        ...
        Debug.Log(string.Join(", ", asset.wayPoints.Select(wp => wp.pos)));
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        foreach (var wayPoint in asset.wayPoints)
        {
            var pos = new Vector3(wayPoint.pos.x, 0, wayPoint.pos.y);
            Gizmos.color = new Color(1f, 0f, 0f, 0.5f);
            Gizmos.DrawSphere(pos, 0.2f);
            
            foreach (var link in wayPoint.link)
            {
                var otherPosXZ = asset.wayPoints[link].pos;
                var otherPos = new Vector3(otherPosXZ.x, 0, otherPosXZ.y);
